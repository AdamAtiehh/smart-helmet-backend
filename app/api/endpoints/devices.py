...
async def list_my_devices(
    ...
    """
    devices = await DevicesRepo.get_user_devices(db, uid)
    return devices
...
async def register_device(
    ...
    if device:
        ...
        if device.user_id and device.user_id != uid:
             raise HTTPException(status_code=400, detail="Device already registered to another user")
        
        # Update owner
        device = await DevicesRepo.update_device(db, device_in.device_id, user_id=uid)
    else:
        # Create new device
...
async def get_device_details(
    ...
    # Security check: ensure user owns the device
